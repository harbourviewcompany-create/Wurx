-- complete_booking locked with `FOR UPDATE` over a LEFT JOIN, which Postgres
-- rejects: "FOR UPDATE cannot be applied to the nullable side of an outer
-- join". The function therefore raised on every call and no booking could ever
-- be completed (no minutes consumed, no provider earnings). Lock only the
-- bookings row, which is all we actually need.
